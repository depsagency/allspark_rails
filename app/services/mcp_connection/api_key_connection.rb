module McpConnection
  class ApiKeyConnection < Base
    def initialize(mcp_server)
      super
      @api_key = mcp_server.credentials&.dig('api_key')
      
      raise AuthenticationError, "API key not configured" if @api_key.blank?
    end

    def connect
      @connected = true
      Rails.logger.debug "[MCP] Connected to #{@mcp_server.name} with API key authentication"
      true
    end
    ...
    def build_authenticated_request(payload)
      ...
      # Add API key to headers based on server configuration
      auth_header = @mcp_server.config&.dig('auth_header') || 'X-API-Key'
      request[auth_header] = @api_key
